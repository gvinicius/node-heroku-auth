...
beforeEach(
  async () => {
    await bcrypt.hash(password, saltRounds).then((hash) => {
      User.collection.create({ email, password: hash, verificationToken, _id: id}).then( (user) =>this.user = user );
    });
  });
...
describe('When to process user update', () => {
  ...
  it('does not update params for the targeted user with invalid params', async (done) => {
    ...
    const res = await testConfig.request(testConfig.app).put('/user/' + wrongId).set('Authorization', `Bearer ${token}`).send({params});
